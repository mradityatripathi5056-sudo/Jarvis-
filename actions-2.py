...
def _focus_window_containing(*substrings: str) -> bool:
    """Kisi bhi visible window ko dhoondh ke focus (foreground) kar deta hai
    jiske title mein diye gaye substrings mein se koi bhi ho. Sirf Windows
    pe kaam karta hai (pywin32 chahiye). Return True = focus ho gaya."""
    ...
    try:
        ...
        def _enum_handler(hwnd, _):
            ...
            if any(s.lower() in title.lower() for s in substrings):
                target_hwnd = hwnd
...
def git_push(params: dict) -> str:
    """'github update karo' / 'sab kuch push kar do' - GIT_REPO_PATH
    (default: Jarvis ki apni folder) mein git add + commit + push chalata
    hai. Agar koi change hi nahi hai to safely bata deta hai, error nahi
    deta. Custom commit message chahiye ho to params mein "message" bhejo."""
    repo_path = config.GIT_REPO_PATH
    message = params.get("message") or "update"
    ...
    try:
        ...
        if status.returncode != 0:
            return f"Git repo nahi mila ya error aaya: {status.stderr.strip()}"

        if not status.stdout.strip():
            # Koi local change nahi hai, phir bhi ho sakta hai push pending ho
            push = _run(["git", "push"])
            if push.returncode != 0:
                return f"Koi naya change nahi tha. Push try kiya to error: {push.stderr.strip()}"
            return "Koi naya change nahi tha, sab already up-to-date hai GitHub par."

        add = _run(["git", "add", "."])
        if add.returncode != 0:
            return f"'git add' fail ho gaya: {add.stderr.strip()}"

        commit = _run(["git", "commit", "-m", message])
        if commit.returncode != 0:
            return f"'git commit' fail ho gaya: {commit.stderr.strip()}"

        push = _run(["git", "push"])
        if push.returncode != 0:
            return f"Commit ho gaya lekin push fail ho gaya (internet ya login check karo): {push.stderr.strip()}"

        return "Saari files GitHub par update kar di."
    except FileNotFoundError:
        return "Git installed nahi mila is system par (ya PATH mein nahi hai)."
    except Exception as e:
        return f"GitHub update karte waqt error aaya: {e}"


def click_on_screen(params: dict) -> str:
    """Screenshot leke ek vision-capable AI model (config.VISION_MODEL)
    se poochta hai ki description wali cheez screen par kahan hai, phir
    wahan click kar deta hai. YouTube like/subscribe jaisi buttons ke
    liye use hota hai jinka koi fixed keyboard shortcut nahi hota.
    params: {"description": "YouTube like button (thumbs up icon)"}"""
    description = (params.get("description") or "").strip()
    if not description:
        return "Kya click karna hai wo bataya nahi gaya."
    if not config.VISION_MODEL:
        return (
            "Vision model .env mein set nahi hai. VISION_MODEL=... daalo "
            "(jaise qwen/qwen2.5-vl-72b-instruct:free), phir dobara try karo."
        )
    ...
    try:
        ...
        prompt = (
            f"Is screenshot mein '{description}' dhoondo. Uska center point "
            "batao SCREEN ke width aur height ke PERCENTAGE (0-100) mein, "
            'is EXACT JSON format mein, kuch aur text nahi: '
            '{"found": true, "x_percent": 50.0, "y_percent": 50.0} '
            'Agar wo cheez screenshot mein dikh hi nahi rahi to: {"found": false}'
        )
        ...
        if not match:
            return f"Vision model se location samajh nahi aayi: {content[:150]}"
        result = json.loads(match.group(0))

        if not result.get("found"):
            return f"'{description}' screen par nahi mila."

        x = int(width * float(result["x_percent"]) / 100)
        y = int(height * float(result["y_percent"]) / 100)
        pyautogui.click(x, y)
        return f"'{description}' pe click kar diya."
    except Exception as e:
        return f"Screen dekh kar click karne mein error: {e}"


def youtube_like(params: dict) -> str:
    """Chalu YouTube video ko like karta hai - pehle YouTube wali browser
    window ko focus karta hai, phir vision se like button dhoond ke click
    karta hai."""
    _focus_window_containing("YouTube")
    time.sleep(0.3)
    return click_on_screen(
        {"description": "the YouTube like button (thumbs up icon) below the video title"}
    )


def youtube_subscribe(params: dict) -> str:
    """Chalu YouTube channel ko subscribe karta hai (vision se button
    dhoond ke click karta hai)."""
    _focus_window_containing("YouTube")
    time.sleep(0.3)
    return click_on_screen(
        {"description": "the red YouTube Subscribe button below the video"}
    )
...
ACTION_MAP = {
    ...
    "youtube_like": lambda params: youtube_like(params),
    "youtube_subscribe": lambda params: youtube_subscribe(params),
    "click_on_screen": lambda params: click_on_screen(params),
    "git_push": lambda params: git_push(params),
