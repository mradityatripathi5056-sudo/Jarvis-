...
def _download_telegram_file(file_id: str, suffix: str) -> str | None:
    """Telegram file_id se koi bhi file (photo/document) download karke
    local temp path return karta hai."""
    try:
        ...
        logging.error(f"Telegram file download fail: {e}")
        return None
...
def _poll_once(allowed_ids: set) -> None:
    ...
    for update in data.get("result", []):
        ...
        # BUG FIX: pehle yahan "text" not in msg check tha jo VOICE aur
        # PHOTO messages ko (jinme "text" field hoti hi nahi) yahin
        # silently skip kar deta tha, neeche ka voice-handling code kabhi
        # chalta hi nahi tha. Ab sirf poora message hi missing hone pe
        # skip karte hain.
        if not msg:
            continue
        ...
        if "text" in msg:
            ...
            # Photo aayi hai - AI vision se describe karke reply karte hain.
            # Caption ko question ki tarah use karte hain (agar diya ho),
            # warna generic "is mein kya hai" poochte hain.
            if chat_id not in allowed_ids:
                logging.info(f"Telegram: untrusted chat_id {chat_id} se photo ignore ki")
                continue
            _send_reply(chat_id, "Photo dekh raha hoon...")
            # Telegram ek hi photo ke multiple sizes bhejta hai - sabse
            # bada (best quality) wala lete hain.
            best_photo = max(msg["photo"], key=lambda p: p.get("file_size", 0))
            local_path = _download_telegram_file(best_photo["file_id"], ".jpg")
            if not local_path:
                _send_reply(chat_id, "Photo download nahi ho payi.")
                continue
            caption = (msg.get("caption") or "").strip()
            question = caption if caption else "Is image mein kya hai, detail mein batao."
            try:
                import skills.vision_skill as _vision
                result = _vision.describe_image_file({"path": local_path, "question": question})
            except Exception as e:
                result = f"Photo samajh nahi paya: {e}"
            finally:
                try:
