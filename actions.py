...
def open_app(app_name: str) -> str:
    ...
    # File manager/explorer alag handle karo - ye "app" nahi, OS ka apna
    # file browser hai, os.startfile(app_name) se nahi khulta.
    if name_key in FILE_MANAGER_NAMES:
...
FILE_MANAGER_NAMES = {
    "file manager", "file explorer", "explorer", "files", "my computer",
    "this pc", "my pc", "folder", "directory",
}


def _open_file_manager(path: str = "") -> str:
    """File manager/explorer kholta hai, optionally kisi specific folder pe."""
    system = platform.system()
    ...
    try:
        ...
        return f"File manager khol diya" + (f" ({target})." if path else ".")
    except Exception as e:
        return f"File manager nahi khul saka: {e}"
...
def take_photo(params: dict) -> str:
    ...
    if not os.path.isabs(filename) and os.path.dirname(filename) == "":
        filename = os.path.join(config.MEDIA_DIR, filename)
...
def screenshot() -> str:
    filename = os.path.join(config.MEDIA_DIR, f"screenshot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
...
ACTION_MAP = {
    ...
    "open_folder": lambda params: _open_file_manager(params.get("path", "")),
