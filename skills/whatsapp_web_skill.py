...
# Playwright driver object (sync_playwright().start() ka return) ko
# GLOBALLY reference rakhna zaroori hai - agar isse sirf local variable mein
# rakha jaaye, to Python garbage collector isse thodi der baad clean kar
# deta hai, aur uske saath poora Chromium browser bhi CHUPCHAAP band ho
# jaata hai (koi error bhi nahi dikhta). Isi wajah se window "cut" ho rahi
# thi - ab isse yahan pakad ke rakh rahe hain taaki jab tak module load
# hai, driver zinda rahe.
_playwright_driver = None

# Playwright ki SYNC API asyncio event loop ke andar chalne wale thread mein
# kaam nahi karti ("Playwright Sync API inside the asyncio event loop" error
# deti hai) - aur Jarvis ka action-executor thread Edge TTS (voice output)
# ki wajah se asyncio loop use karta hai. Isliye WhatsApp ke saare browser
# operations is ALAG, dedicated worker thread mein chalate hain jahan kabhi
# koi asyncio loop set nahi hota - Playwright wahan bina issue ke chalta hai.
...
def _get_executor() -> concurrent.futures.ThreadPoolExecutor:
    ...
    if _executor is None:
        _executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1, thread_name_prefix="whatsapp-playwright"
        )
    return _executor


def _run_in_worker(fn, *args, **kwargs):
    """Diye gaye function ko dedicated Playwright thread mein chalata hai
    aur result wapas laata hai (blocking) - taaki calling thread ka
    asyncio loop Playwright ko disturb na kare."""
    future = _get_executor().submit(fn, *args, **kwargs)
    return future.result()


def _ensure_page():
    global _context, _page, _playwright_driver
    ...
    if _context is None:
        ...
        _context = _playwright_driver.chromium.launch_persistent_context(_PROFILE_DIR, headless=False)
...
def whatsapp_open(params: dict) -> str:
    return _run_in_worker(_whatsapp_open_impl, params)


def _whatsapp_open_impl(params: dict) -> str:
...
def send_whatsapp_message_by_name(params: dict) -> str:
    return _run_in_worker(_send_whatsapp_message_by_name_impl, params)


def _send_whatsapp_message_by_name_impl(params: dict) -> str:
...
def whatsapp_voice_call(params: dict) -> str:
    return _run_in_worker(_whatsapp_voice_call_impl, params)


def _whatsapp_voice_call_impl(params: dict) -> str:
...
def whatsapp_video_call(params: dict) -> str:
    return _run_in_worker(_whatsapp_video_call_impl, params)


def _whatsapp_video_call_impl(params: dict) -> str:
...
def whatsapp_wait_for_reply(params: dict) -> str:
    return _run_in_worker(_whatsapp_wait_for_reply_impl, params)


def _whatsapp_wait_for_reply_impl(params: dict) -> str:
...
def whatsapp_close(params: dict) -> str:
    return _run_in_worker(_whatsapp_close_impl, params)


def _whatsapp_close_impl(params: dict) -> str:
